...
Depending on the amount of sessions etc. we are using this can take some time. But is all this code needed in every session, e. g. in the session the main users are starting for their daily work? I bet no.
...
But how do we get it done to load the code for session_snafu and/or session_foobar? Let's choose gis_stanzas because they are already used for starting sessions. We can set an environment_variable SESSIONS_DIRECTORIES with for example session_foobar:
...
_private _method sessions_loader.load_sessions_specified_in_environment_variable
	...
	##
	## It should be definend in the gis_alias.
...
This solution is easy to implement and if you establish and maintain a clear structural and naming pattern it is easy to remember and support. And it saves time, not only for our customers.